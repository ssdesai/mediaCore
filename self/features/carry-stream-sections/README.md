# Stream-derived report sections survive a re-render

A re-render of a frozen feature's report no longer destroys its three stream-derived
sections — "LoC changed", "Re-hunting" and "Cross-plan edit overlap". `*.stream.jsonl` is
gitignored and lives only in the worktree the runner ran in, so once `feature-start.sh`
prunes that worktree the streams are gone for good. `feature-capture.sh`'s annotate step
re-renders every frozen record whose `also_claimed_by` it changed, and `report.py` then
wrote `not computed: streams unavailable` over the values the committed `report.json`
held. The rewritten files rode the *closing* feature's `<slug>: cost records` commit, so
a PR for one feature edited another's frozen report (humanNetworkMap PRs #116 and #117,
2026-09-23, `source-media-sections` losing "LoC changed 67").

The committed `report.json` is the frozen record of what the streams said: it was
computed while they existed. So `report.py` carries each stream-derived value forward
from it when there is no stream left to compute from. That means per plan for
`plan_length_vs_loc[].loc_changed`, and the whole section for `re_hunting` and
`edit_overlap` when no plan has a stream. A re-render after an annotation then differs
from the committed report only in what the annotation changed, and the same is true of a
`--recapture` from the primary. No guard in `feature-capture.sh` is needed. The fix is at
the one reader that had two sources and used the ephemeral one.

It also adds the `self/BACKLOG.md` entry for the interrupted-start defect this feature's
own start hit.
...
| Plan | What it does |
|---|---|
| `review/incomplete/01-review-opus.md` | Independent review of the hand build against the spec above |

## Deliberately excluded

- **Committing the streams, or copying them into the feature before the prune.** They are
  large raw event logs, gitignored for that reason, and every figure the report needs from
  them is already in the committed `report.json`. Copying them would make a second
  permanent record of what that file already freezes.
- **Some streams present and some gone.** When at least one plan still has a stream,
  re-hunting and edit overlap are computed from the streams that exist, with the existing
  `computed without streams for` warning, exactly as before. That is a live feature
  missing a stream, not a frozen one whose worktree was pruned, and merging a partial
  computation with a carried one has no correct answer. LoC is per plan, so it is carried
  per plan either way.
- **Marking a carried value in the report.** A mark would itself be a diff on every
  re-render, which is the defect. The carried value is the same fact the report already
  stated.
...
```json
{
  "slug": "carry-stream-sections",
  "method": "hand",
  "plans": ["01-review-opus"],
  "branches": ["carry-stream-sections"],
  "base": "main",
  "session_window": {"from": "2026-09-23T16:12:21Z", "to": "2026-09-23T16:20:23Z"},
  "exclude_sessions": [],
  "exclude_subagents": [],
  "sessions": [],
  "subagents": []
