# Hook: deny a chained cd, expand simple brace lists

`CONVENTIONS.md` § Shell commands forbids chaining `cd` with another command, and nothing
enforced it. `hooks/allow-repo-commands.sh` either approved such a chain or stayed silent,
so each miss cost the human an approval and taught the model nothing. This feature makes
the hook **deny** any command that chains `cd`/`pushd` with another, with a reason naming
the rewrite, so the model corrects itself on the next call. In the same session a plain
repo-confined `ls …/{auto,verify,review}/…` prompted because the hook refuses every
unquoted brace. The hook now **expands simple brace lists** and checks each word, so
reads like that are approved. That second part widens a security policy, at the user's
decision.
...
| Plan | What it does |
|---|---|
| `auto/incomplete/101-hook-tests-sonnet.md` | Rewrites `self/tests/allow-repo-commands.sh`: cd chains move to a `DENY` group, their unchained rewrites to `ALLOW`, plus `NOT_DENIED`, brace allow/refuse groups and a deny-reason check. RED until 102. |
| `auto/incomplete/102-hook-deny-and-braces-opus.md` | `chains_chdir` and the deny decision; brace expansion (raw-word rule, literal and expanded argv both pass, 256-word cap); docstring, `hooks/README.md`, root `README.md` row, `CONVENTIONS.md`, `sync-plans.sh` comment. |
| `verify/incomplete/103-verify-sonnet.md` | Probes brace expansion against real bash for approvals bash would not grant; replays recent real transcript commands for false denies. |
| `review/incomplete/104-review-opus.md` | Reads the diff against the threat model, the no-false-deny rule and the docs. |

Single level: plan 101 is the acceptance test, and the final gate owns it.

## Deliberately excluded

- **Denying other avoidable shapes** (`$HOME` in a path, redirects, `~`). Unlike a chained
  `cd`, each also appears in legitimate write commands the human would approve, so a deny
  would block real work. They keep falling through to the prompt.
- **Nested, quoted or escaped braces, and sequence expressions.** Matching bash exactly
  there needs a real word parser in place of `shlex`. They still prompt, which costs one
  approval.
- **`feature-start.sh`'s printed `cd $WORKTREE && claude` hint.** It is for a human typing
  in a terminal, not a Bash tool call, so the hook never sees it.
- **Wiring the hook into agentTooling's own checkout.** This repo has no
  `.claude/settings.json`. The hook reaches consuming repos through `sync-plans.sh`, as
  before.
...
```json
{
  "slug": "hook-cd-deny-braces",
  "method": "plans",
  "plans": ["101-hook-tests-sonnet", "102-hook-deny-and-braces-opus", "103-verify-sonnet", "104-review-opus"],
  "branches": ["hook-cd-deny-braces"],
  "base": "main",
  "session_window": {"from": "2026-09-16T14:17:21Z", "to": "2026-09-16T14:50:38Z"},
  "exclude_sessions": [],
  "exclude_subagents": [],
  "sessions": ["391e7b6b-aa5d-4058-844d-d71175291420"],
  "subagents": []
}
```
