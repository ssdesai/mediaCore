# A stale primary is updated before a feature starts

On 2026-09-21 a consuming repo's session hit two failures starting a feature. First, its
primary's `main` lagged `origin/main`, so the primary's old `feature-start.sh` wrote and
`git add`-ed a routing-record path that the new branch's `routing.py` no longer writes, and
the `S: start` commit failed half-way. Now, when the primary's `main` is behind
`origin/main`, `feature-start.sh` fast-forwards it, starts nothing, and exits 3 with the
command to run again. It refuses, touching nothing, when the primary is off `main`, has
diverged, or has a local change in the way. Second, the git-deny reason told the session
that only the human runs `feature-start.sh`, so it built in the primary instead. The reason
now says the session may run it, and must do so before editing.

## Plans

Built by hand; the review is the only plan.

| Plan | What it does |
|---|---|
| `review/incomplete/01-review-opus.md` | Independent review of the stale-primary check and the deny-reason text against the spec |

## Deliberately excluded

- **Re-exec'ing the updated script automatically.** The user chose an explicit exit and
  rerun: a start that moves `main` says so and stops, rather than quietly restarting.
- **A hook that denies Edit/Write in the primary checkout.** It would enforce "start
  before you edit" mechanically, but it also blocks legitimate edits on `main`. It is held
  until the corrected deny reason is shown not to be enough.
- **Checking against `--base` instead of `origin/main`.** The primary tracks `main`, so
  `origin/main` is what the running copy should match, whatever the feature branches from.

## Machine-readable

```json
{
  "slug": "start-refreshes-main",
  "method": "hand",
  "plans": ["01-review-opus"],
  "branches": ["start-refreshes-main"],
  "base": "main",
  "session_window": {"from": "2026-09-21T17:16:23Z", "to": "2026-09-21T17:39:09Z"},
  "exclude_sessions": [],
  "exclude_subagents": [],
  "sessions": ["6de7afed-1b5b-450b-82f7-7f3bc9a6e675"],
  "subagents": []
}
```
