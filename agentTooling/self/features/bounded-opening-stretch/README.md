# Bounded opening stretch

A session claimed by several features is split by the windows they hold it with, and
the stretch before the earliest window opens goes to the earliest claimant alone, on
the reasoning that a session's opening is the planning for the first feature it
started. That is true of minutes and false of days: session `2d8b1236` ran for 45 hours
coordinating fifteen features in three other repos before the first of its three
agentTooling claimants opened, and `stale-failed-sidecars`, earliest by 49 seconds with
a 25-minute window, was paid $50.12 of that head. This feature bounds the fallback: the
head is paid to the earliest claimant only as far back as that claimant's own window is
long, and beyond that it falls to the unclaimed remainder, disclosed with its own figure
and the remedy named — as the tail already is. Built direct (`AGENT_DIRECT.md`): one
opus implementer from `brief.md`, reviewed from a brief written before the build.

## Plans

| Plan | What it does |
|---|---|
| `brief.md` | The implementer's brief: the defect, the rulings, the assertions that must go red first. Not a plan; recorded for resume. |
| `NOTES.md` | The implementer's rulings and the mutation each new assertion was proved red against. |
| `CHECKPOINT.md` | The shape of the build, at `status: committed` — the direct feature's `auto/complete/`. |
| `review/incomplete/99-review-opus.md` | Independent review of the diff against `main`, from the spec: one bound in one place, the ranking pool unchanged, the sum invariants on every path, the disclosure actionable. |

## Deliberately excluded

- **Pinning `2d8b1236` into the fifteen features it coordinated.** Fifteen recaptures
  across four repos against ageing transcripts, and the head would still go to whichever
  sorted first. The unclaimed remainder with its remedy named is the honest record; a pin
  can follow if anyone wants the dollars attributed.
- **A fixed grace period.** The head is planning for the feature that follows, and that
  scales with the feature; a fixed hour pays a two-minute feature an hour it did not plan.
- **A new `planning.json` field for the head.** The existing `unclaimed_usd` and
  `unclaimed_duration_s` hold it; the warning tells the two apart.
- **Re-capturing the three `2d8b1236` claimants.** The coordinator's step after this
  lands.
...
```json
{
  "slug": "bounded-opening-stretch",
  "method": "direct",
  "plans": ["99-review-opus"],
  "branches": ["bounded-opening-stretch"],
  "base": "main",
  "session_window": {"from": "2026-09-10T03:48:30Z", "to": "2026-09-10T04:13:55Z"},
  "exclude_sessions": [],
  "exclude_subagents": [],
  "sessions": ["e21abdc0-6f3b-4306-a18e-81272cadb107"],
  "subagents": ["a32a0bdbdc43a1740", "af85e124faead3c20"]
}
```
