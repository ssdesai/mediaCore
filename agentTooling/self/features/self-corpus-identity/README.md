...
A capture run from a consuming repo's vendored `agentTooling/` derived the identity of
agentTooling's own corpus by asking `git` about `<consumer>/agentTooling/`, which resolves
to the consumer's origin. So every self-corpus feature entered the consumer's claim set
as `<consumer>/<slug>` while the shared ledger holds it as `agentTooling/<slug>`; the
dedupe key is `(repo, slug)`, both survived, and one feature was counted twice under a
name that does not exist. Measured on session `ed088063`: a simulated vinylCatalogue
capture yielded 13 intervals including phantom `vinylCatalogue/recovered-duration-lower-bound`
and `vinylCatalogue/tooling-backlog-2026-09-06`, $3.89 mis-booked across six features.
This feature declares the self corpus's identity in `analysis/roots.py` — it can only
ever belong to agentTooling — and routes every `--self` "which repo is this" question
through it, so a `--self` operation never asks `git` who it is. Built direct
...
| `review/incomplete/98-review-opus.md` | Independent review of the diff against `main`, from the spec: one identity rule per corpus, the key is the whole identity, nothing frozen moves, the standalone answer is unchanged. |

## Deliberately excluded

- **A ledger migration.** Every agentTooling session claim in `~/.claude/subagent-claims.json`
  already carries `https://github.com/ssdesai/agentTooling.git`, because the standalone
  checkout's origin is that URL; declaring the same string changes no existing key.
- **Weakening the dedupe to `repo_name`.** Two repos may share a display name; the whole
  identity is the key, and the fix is to make the manifest's identity equal the
  ledger's.
- **Re-capturing the consumers' affected records.** The coordinator's step after this
  lands: `update.sh` in each consumer, then the `ed088063` two-pass `--recapture`.
- **`manifest_pinned_subagents`' slug-only lookup.** It stays; only its stated rationale
  changes.
...
```json
{
  "slug": "self-corpus-identity",
  "method": "direct",
  "plans": ["98-review-opus"],
  "branches": ["self-corpus-identity"],
  "base": "main",
  "session_window": {"from": "2026-09-09T22:09:13Z", "to": "2026-09-09T22:34:32Z"},
  "exclude_sessions": [],
  "exclude_subagents": [],
  "sessions": ["e21abdc0-6f3b-4306-a18e-81272cadb107"],
  "subagents": ["a817ca734695abc99", "a8b1d88969546bed9"]
}
```
