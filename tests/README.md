...
- `assets/` — frozen inputs, never generated and never regenerated; see its README.
  Currently `its-saxy-schema-1/release.json`, the bundle `mediacore` 0.2.0 wrote, and
  `its-saxy-schema-2/release.json`, the one 0.3.0 wrote. `test_bundle.py` reads both to
  prove this install still reads older bundles — and re-writes them, to prove the writer
  relabels them; `test_release.py` validates the schema-2 one through the model.
...
- `test_fixture.py` — the batch's acceptance test: loads `fixtures/its-saxy/` through
  ...
  plus every file hash, at `schema_version` 3, with no track carrying an `artist`
  (§11 records one release artist and no per-track ones) and `original_year` present
  on disk as `null` (§11 does not say the pressing is a reissue). Depends on
  `fixtures/its-saxy/` having been generated by `scripts/make_fixture_its_saxy.py` — it
  is red on a checkout where that has not run.
...
- `test_release.py` — the §3 models plus the one fact about the package itself that
  belongs beside them: `mediacore.__version__` equals
  `importlib.metadata.version("mediacore")`, so the two hand-kept copies of the version
  (`__init__.py` and `pyproject.toml`, with the `v0.4.0` tag consumers pin as a third)
  cannot drift — the gate installs the package, so the metadata side is real — and
  equals the literal `CURRENT_PACKAGE_VERSION` (0.4.0). Then the
  models: JSON round-trip, `extra="forbid"`, closed vocabularies, defaults, ref
  validation on nested models, `Track.artist` (defaults to
  `None`; set and unset tracks round-trip through `model_dump(mode="json")`; `artist` on
  a `Credit` is an extra, a track artist being no role credit), `Release.original_year`
  (defaults to `None` and dumps as a present `null`; round-trips set and unset; sits
  right after `released`; accepts 1877 and the current year, rejects 1876, 0, -1 and
  next year — through `Release(...)` and `model_validate` alike; rejects a value later
  than `year`, accepts one equal to it and one with `year` absent; and the frozen
  `assets/its-saxy-schema-2/release.json`, which has no such key, validates with it
  `None`), `SCHEMA_VERSION` pinned
  at 3 both symbolically and as a literal (`CURRENT_SCHEMA_VERSION`) so a bump is a
  deliberate edit here, and the `file`/`sha256`
...
- `test_bundle.py` — `read_bundle` / `write_bundle` including every failure case
  ...
  newer than `SCHEMA_VERSION` (equal passes, newer raises, and schema **4** — one past
  this install — is named explicitly), a written bundle being stamped **3**
  (`CURRENT_SCHEMA_VERSION`, a literal), and `verify=True` checking
  each `AudioFile`'s actual byte size against `size_bytes` (`verify=False` skips it).
  Also `Track.artist` across the disk boundary: it survives `write_bundle` →
  `read_bundle`, a track written without one reads back `None`, an absent artist is
  serialised as `"artist": null` rather than omitted (the writer dumps the model whole),
  and `assets/its-saxy-schema-1/` — a schema-1 bundle with no `artist` key anywhere —
  still reads through `read_bundle(..., verify=False)` with every track's artist `None`.
  That asset is read in place rather than copied: `verify=False` touches no media, so
  the frozen `release.json` is the whole asset. Writing it back out is pinned too: the
  writer **stamps** `schema_version`, so that schema-1 release re-written by this
  install lands labelled with the current version with an `artist` key on every track,
  while the release the reader returned still says 1 (§13, 2026-09-06). It is written
  with `media` and `audio` emptied — the asset carries no media bytes to hand
  `write_bundle`. `Release.original_year` gets the same treatment (§13, 2026-09-24): it
  survives `write_bundle` → `read_bundle`, an absent one is written as
  `"original_year": null`, and `assets/its-saxy-schema-2/` — no `original_year` key —
  reads with it `None` and re-writes labelled 3 with the key present as `null`, while
  the model read still says 2.
...
- `test_store.py` — the §5.1 bundle store. Every behavioural test runs **twice**, against
  ...
  **version** — `<record>/<timestamp>` — including a same-second re-export whose slug
  differs and one whose `exported_at` differs only in microseconds; `put` stamping
  `schema_version`, so a release read at schema 1 is stored, listed and re-read at this
  install's `SCHEMA_VERSION` on both backends; the `list`/`open` asymmetry on a newer `schema_version` (listed with
  its version — even carrying a field
