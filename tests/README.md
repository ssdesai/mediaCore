...
- `assets/` — frozen inputs, never generated and never regenerated; see its README.
  ...
  `test_bundle.py` reads to prove a 0.3.0 reader still reads a schema-1 bundle.
...
- `test_release.py` — the §3 models: JSON round-trip, `extra="forbid"`, closed
  vocabularies, defaults, ref validation on nested models, `Track.artist` (defaults to
...
- `test_bundle.py` — `read_bundle` / `write_bundle` including every failure case
  ...
  the frozen `release.json` is the whole asset.
