...
- `conftest.py` — shared builders: `sha256_bytes`, `make_release(**overrides)` (a
  ...
  `files` argument, and the bundle store fixtures: `file_store_uri`, `s3_store_uri`
  (imports `boto3` and `moto` inside its body so a missing `[dev]` extra fails only the
  s3 tests, not collection), the parametrized `store_uri` (runs a test against both
  backends), and `its_saxy_source() -> (Release, {sha256: Path})` for `BundleStore.put`.
  Constants: `SAMPLE_EXPORTED_AT`, `SAMPLE_RECORD_ID`, `SAMPLE_RELEASE_REFS`,
  `SAMPLE_ARTIST_REFS`, `S3_TEST_BUCKET`, `S3_TEST_PREFIX`, `S3_TEST_REGION`,
  `S3_TEST_CREDENTIAL`.
