...
- `its-saxy-schema-1/release.json` — the IT'S SAXY bundle's `release.json` exactly as
  ...
  bundle and defaults every `Track.artist` to `None` (`INTEGRATION.md` §12, §13
  2026-09-06). It is the `release.json` alone: `verify=False` touches no media, so the
  bundle's `media/` files are deliberately not duplicated under `tests/`.
